...
class AvailableTest:
	...
	async def _popen_connect(self, node: Node, port: int):
		...
		config = json5.loads(json5.dumps(self.CONFIG_TEMPLATE, quote_keys=True, trailing_commas=False))
	...
	async def _http_connect(self, node: Node):
		...
		response_times = 0
		url = random.choice(self.url_list)
		host = node.settings['vnext'][0]['address']
		port = node.settings['vnext'][0]['port']
		for i in range(self.times):
			await asyncio.sleep(self.sleep_seconds)
			try:
				begin_time = time.time()
...
class V2Ray:
	...
	async def set_config(self, nodes: list):
		if self.template:
			if isinstance(self.template, dict):
				data = json5.dumps(self.template, quote_keys=True, trailing_commas=False)
			else:
				data = self.template
		else:
			...
			data = file.read()
			file.close()
		config_json = json5.loads(data)

		handler = self.config_object.build(config_json, nodes=nodes)
		for middleware in self.middleware_list:
			handler = middleware(config_json, handler)
		await handler
		new_config = json5.dumps(config_json, quote_keys=True, trailing_commas=False, indent=4, sort_keys=True)
	...
	async def run(self):
		while True:
			...
			if self.restart_command:
				logging.info(self.restart_command)
				p = subprocess.Popen(self.restart_command, shell=True
	...
	def decode_subscribe(cls, subscription_content: str):
		...
		config_list = [json5.loads(base64.urlsafe_b64decode(line.replace(b"vmess://", b""))) for line in vmess_lines]
		return config_list
